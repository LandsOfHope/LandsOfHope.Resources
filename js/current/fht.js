...
getObj('Buttons').innerHTML = Adr('window.location.replace(\'fhtourn.asp?CharsAt=' + CharsAt + '&t=' + Infos[v].z + '\');','Enter','Enter');
}

function GetTips(v) {
 return '<b>' + Infos[v].i + '</b>' + (Infos[v].h > 0 ? '<br>Player Hosted' : '') + '<br>Tourney Type: ' + (Infos[v].t == 'FREEFORALL' ? 'Free For All<br>The person with the highest wins at the end of the Tournament wins.' : (Infos[v].t == 'BOUNTY' ? 'Bounty<br>The person with the highest prize fund at the end of the Tournament wins.' : 'Deathmatch<br>The person with the highest wins at the end of the Tournament wins. If you lose a match you are knocked out of this tournament and can not participate till it resets.')) + '<br>Basic Prize Fund: ' + window.top.BSGM(Infos[v].m) + '<br>This is the basic amount the Tourney winner will receive, not including any Prize money.'
}
