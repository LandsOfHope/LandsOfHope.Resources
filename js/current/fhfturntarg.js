...
function AC(v, Picture, Itty, q, m, g, rs, dot) {
	...
	document.write('<div id="Choice' + sc + '" title="' + Itty + '" style="width: 40px; height: 40px; padding: 1px; margin: 1px; background-color: ' + Color + '; float: left;" onclick="DC(' + sc + ')" onmouseover="PC(' + sc + ')" onmouseout="RC(' + sc + ')"><img src="' + IPath + (Picture == '' ? 'na.gif' : Picture) + '" width=40 height=40></div>');
	sc = sc + 1;
}
