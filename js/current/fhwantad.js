...
function DC(v) {
	getObj('Stuff2').innerHTML = Tipsfor(v) + '<input type=hidden name="WN" value="' +  (Infos[v].i =='' ? Infos[v].m : Infos[v].i) + '"><input type=hidden name="WI" value="' +  Infos[v].v + '"><input type=hidden name="WP" value="' + Infos[v].p + '"><input type=hidden name="OID" value="' + Infos[v].mid + '"><table class="weakcell">' + (Infos[v].nl == 0 ? '<tr><td>Item Level: </td><td><input name=Level id=Level size=4 maxlength=4 value="' +Infos[v].l + '" onkeypress="return fxkp(event);"> or <input type=checkbox name=Level2 value=1> any level</td></tr>' : '<input name=Level id=Level type=hidden value="' +Infos[v].l + '"><input type=hidden name=Level2 value=1>') + '<tr><td>Quantity: </td><td><input name=Quantity size=3 maxlength=3 value="1" onkeypress="return fxkp(event);"></td></tr><tr><td>Price: </td><td>' + FormMoney('Price',0) + '</td></tr><tr><td colspan=2>If this job can be completed multiple times enter the amount of money you wish to spend in total below, otherwise leave it as 0.</td></tr><tr><td>Fund: </td><td>' + FormMoney('Fund',0) + '</td></tr><tr><td colspan=2>' + Adr('if (Processing == 0) {if (Math.abs(getObj(\'Level\').value) >= ' + Infos[v].l + ') {if (Math.abs(getObj(\'Price\').value) >0) {Processing = 1; getObj(\'Take\').submit();} else {alert(\'Please enter a price you are willing to pay for the completion of this job\');}} else {alert(\'This Item has a minimum level of ' + Infos[v].l + ', you entered a level too low. Try again\')}}', 'Create Job Listing', 'Create Job') + Adr('window.top.loadwindow2(\'imi.asp?Test=' + Infos[v].v + '&Bonus=0&Material=\',300,300,\'iwindow\',\'' + Infos[v].i + '\');','Info','Info') + '</td></tr></table>'
