...
function DC(v) {
	getObj('Stuff2').innerHTML = '<b>' + Infos[v].t + '</b><br>Dungeon Mastery Skill: ' + Infos[v].s + '<br>Effect: ' + Infos[v].d;
	getObj('Pic').innerHTML = DrawImage2(IPath + (Infos[v].p == null ? 'na.gif' : Infos[v].p), 0, 0) 
	getObj('Buttons').innerHTML = Adr('if (Processing == 0) {Processing = 1; window.location.replace(\'?CharsAt2=' + Infos[v].v + '\');}', 'Set', 'Set');
}
