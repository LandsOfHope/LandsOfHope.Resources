...
function DrawRoom(x, y, z, e, p) {
	...
	if (Bz[x][y].nm == 0 || Bz[x][y].nm == 2) {
		strmap += " " + (Bz[x][y].nm == 0 ? "style=\"background-color: " + Bz[x][y].Color + ";cursor:pointer;\" onmouseover=\"InfoMap('" + Bz[x][y].Picture + "','"  + Bz[x][y].ObjectM + "','" + Bz[x][y].Color + "','<b>" + Bz[x][y].Caption + "</b>" + (Bz[x][y].Hidden != 0 ? "<br>Stealth: " + Bz[x][y].Hidden : "") + "<br>X: " + x + " Y: " + y + " Floor: " + lngstartz + "');\" onmouseout=\"hideTip();\" " : "style=\"background-color: " + Bz[x][y].Color + ";\" ");
	} else {
		strmap += " ";
	}
	if (Bz[x][y].Picture != '') {
...
function MBg(BuildingID2,x,y ,go2b, p, e) {
	...
	if (BuildingID2 == 0) {
		...
		window.frames['ResultsOfit'].location.replace("fhblme3.asp?x=" + x + "&Special=" + Special + "&z=" + Floor + "&y=" + y + "&CharsAt=" + BuildingID2)
	} else {
		InfoTip('', 'Clicked ' + TileName);
		var v = 0;
		var i = 0;
		for (i = 0; i < getObj('Draw2').length; i++)  {
			if (getObj('Draw2')[i].checked == true) {
				v = getObj('Draw2')[i].value
				var strurl = "?x=" + x + "&Special=" + Special + "&z=" + Floor + "&y=" + y + "&CharsAt=" + BuildingID2;
				if (v == 1) {
					strurl = "fhblme2.asp?Special=" + Special + "&CharsAt=" + BuildingID2;
				} else if (v == 6) {
					strurl = "fhblmep.asp?Special=" + Special + "&CharsAt=" + BuildingID2;
				} else if (v == 2) {
					strurl = "fhblme3.asp" + strurl;
				} else if (v == 3) {
					strurl = "fhblmed.asp" + strurl;
				} else if (v == 5) {
					strurl = "fhblmef.asp" + strurl;
				} else if (v == 7) {
					strurl = "fhblmec.asp" + strurl;
				} else if (v == 20) {
					strurl = "fhblmes.asp" + strurl;
				} else if (v == 25) {
					strurl = "fhblmepo.asp" + strurl;
				} else if (v == 21) {
					strurl = "fhblmek.asp" + strurl;
				} else if (v == 22) {
					strurl = "fhblmpk.asp" + strurl;
				} else {
					strurl = "fhblme.asp?Special=" + Special + "&CharsAt=" + BuildingID2;
				}
...
var v = 0;
var i = 0;
strTest += '<tr><td colspan=2 class="navmenu2" style="font-size: 9px; height: 12; padding: 0px; margin: 0px">Floor / View</td></tr>';   
...
strTest += '</table></td><td><table class="itemtext" cellpadding=0 cellspacing=0>';        
...
strTest += '<tr><td colspan=2 class="navmenu2" style="font-size: 9px; height: 12; padding: 0px; margin: 0px">Action</td></tr>';   
strTest += MakeAction('Edit',0);
strTest += MakeAction('Keys',21);
strTest += MakeAction('Image',1);
strTest += MakeAction('Preview',6);
strTest += MakeAction('Build',2);

if (isDungeon == 0) {
	strTest += MakeAction('Spawn',20);
} else {
	strTest += MakeAction('Spawn Key',22);
	strTest += MakeAction('Portal',25);
}
strTest += MakeAction('Items',5);
strTest += MakeAction('Color',7);
strTest += MakeAction('Delete',3);
...
function MakeAction(strA, avalue) {
	return '<tr><td align=right>' + strA + '</td><td><input class="btn" name=Draw2 id=Draw2 type=radio ' + (deffun == avalue ? 'checked' : '') + ' value=' + avalue + ' style="width:15; background-Color: ' + BGCOLOR_S + '; border: 1px outset ' + BORDER1_S + '; font-weight: bold"></td></tr>';    
}
...
function GoP(PageNo) {
	var d = GetDef();
	window.location.replace("fhblm.asp?CharsAt=" + CharsAt + "&deffun=" + d + "&Special=" + (Special >= 10 ? 10 : 0) + "&StartX=" + xx + "&StartY=" + yy + "&TestZ=" + PageNo);
}
...
function GoXY(xx,yy) {
	...
	var d = GetDef();
	window.location.replace("fhblm.asp?CharsAt=" + CharsAt + "&deffun=" + d + "&Special=" + (Special >= 10 ? 10 : 0) + "&StartX=" + xx + "&StartY=" + yy + "&TestZ=" + Floor);
}
