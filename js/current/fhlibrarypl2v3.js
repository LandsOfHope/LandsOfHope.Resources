...
function AvC(Color, q, m, mt, PictureID, l, v, t, i, mid, lh, ad, ivi) {
	var q2 = '';
	if (q == 0 || q == 1) { q2 = 'Common' }
	else if (q == 2 || q == 3 || q == 4) { q2 = 'Uncommon' }
	else if (q == 5 || q == 6 || q == 7) { q2 = 'Rare' }
	else if (q == 8 || q == 9) { q2 = 'Very Rare' }
	else if (q == 10) { q2 = 'Unique' }
	else if (q == 11) { q2 = 'Artifact' }
...
function Tipsfor(v) {
	return '<b style=\'color: ' + Infos[v].c + '\'>' + Infos[v].m + '</b><br>Makes: ' + (Infos[v].i == '' ? Infos[v].m : Infos[v].i) + '<br>Skill Name: ' + MT + '<br>Skill Required: ' + Infos[v].mt + '<br>Rarity: ' + Infos[v].q2 + '<br>Availability: Looted' + (Infos[v].lh == 0 ? (Infos[v].t == 0 ? '' : ' or Trainer') : '') + '<br>Equip/Use Level: ' + Infos[v].l + '<br>' + (Infos[v].ad != 0 ? 'Allow Deconstruction: Yes' : 'Allow Deconstruction: No') + (Infos[v].q == 11 ? '<br><b style=\'color: ' + Infos[v].c + '\'>Extremely Rare Artifact</b>' : '');
}
