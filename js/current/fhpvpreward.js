...
function AvC(rid, rt, rn, rpp, rp, rf, rc, rl, ru, rq) {
	...
	document.write('<tr id="I' + IC + '" onmouseover="PC(' + IC + ')" onmouseout="RC(' + IC + ')"  onclick="DC(' + IC + ')"><td><table width=370 class="weakercell" cellpadding=0 cellspacing=0><tr><td rowspan=2><img width=30 height=30 src="' + (IPath + rpp + '/' + PictureID) + '"></td><td style="color: ' + Color + '"><b>' + rn + (rq > 1 ? ' * ' + rq : '') + '</b></td></tr><tr><td><table class="weakercell" width=340 cellpadding=0 cellspacing=0><td width=90>Fame: ' + rf + '</td><td width=160>' + window.top.BSGM3(rc) + '</td><td width=80>Level: ' + rl + '</td></tr></table></td></tr></table></td></tr>');
...
function Tipsfor(v) {
	return '<b>' + Infos[v].rn + '</b><br>Quantity: ' + Infos[v].rq + '<br>Unique: ' + (Infos[v].ru != 0 ? 'Yes' : 'Nope') + '<br>Fame: ' + (Fame >= Infos[v].rf ? '<font id=tgreen>' + Infos[v].rf + '</font>' : '<font id=tred>' + Infos[v].rf + '</font>') + '<br>Cost: ' + (Money >= Infos[v].rc ? '<font id=tgreen>' + window.top.BSGM3(Infos[v].rc) + '</font>' : '<font id=tred>' + window.top.BSGM3(Infos[v].rc) + '</font>') + '<br>Level: ' + (Level >= Infos[v].rl ? '<font id=tgreen>' + Infos[v].rl + '</font>' : '<font id=tred>' + Infos[v].rl + '</font>');
}

function DC(v) {
	getObj('Stuff2').innerHTML = Tipsfor(v)
	getObj('Pic').innerHTML = "<img src='" + (Infos[v].p == '' ? 'na.gif' : Infos[v].p) + "'>";
	getObj('Buttons').innerHTML = (Level >= Infos[v].rl && Money >= Infos[v].rc && Fame >= Infos[v].rf ? '<' + strClicky + ' onclick="if (Processing == 0) {window.location.replace(\'?P=' + PageNo + '&CharsAt=' + CharsAt + '&Type=' + Infos[v].rid + '\')}" style=\'width: 65\'>Take</button>' : '') + '<' + strClicky + ' onclick="if (Processing == 0) {window.top.loadwindow2(\'' + (Infos[v].rt == 1 ? 'imi.asp' : (Infos[v].rt == 3 ? 'imiv.asp' : 'Mz.asp')) + '?Test=' + Infos[v].rid + '\',300,300,\'iwindow\',\'' + Infos[v].rn + '\')}" style=\'width: 65\'>Info</button>';
}

function FBI(v) {
	window.location.replace('?CharsAt=' + CharsAt);
}
