...
function GS(in3, in2) {
	if (Processing == 0) {
		Processing = 1;
		window.top.Interface.location.replace('/gm/gmBuildings.asp?CharsAt=' + in2 + '&P=' + PageNo + '&Type=' + in3 + '&Mask=' + Mask)
	}
}
...
function DC(v) {
	...
	getObj('Stuff2').innerHTML = '<b>' + Infos[v].i + '</b><br>Rooms: ' + Infos[v].r + '<br>Location: ' + Infos[v].x + ', ' + Infos[v].y + ' ' + GetRealm(Infos[v].g) + '<br>' + (Infos[v].a != 0 ? 'You own this ' : 'You manage this ') + 'Building' + (Infos[v].h != 0 ? '<br><i>Stealthed</i>' : '') + (Math.round(Infos[v].f) < 0 ? '<br><font id=tred>Collapsing, insert funds</font>' : '') + (Infos[v].hd == 0 ? '<br><font id=tred>Can not be Locked</font>' : '<br>Lockable');
...
function AB(PictureID, ItemName, ItemID, Color, h, l, x, y, v, g, c, f, d, hd, r, ls) {
	...
	Infos[IC] = new newInfo(PictureID, ItemName, ItemID, Color, h, l, x, y, v, g, c, f, d, hd, r, ls);
	//v=' + ItemID + ' ls=' + ls + ' i="' + ItemName + '" r='+ r  + ' z=' + v + ' hd=' + hd + ' d=' + d + ' h=' + h + ' f=' + f + ' a=' + c + '
	// l=' + l + ' g=' + g + ' x=' + x + ' y=' + y + '
...
function newInfo(PictureID, ItemName, ItemID, Color, h, l, x, y, v, g, c, f, d, hd, r, ls) {
	this.color = Color;
	this.i = ItemName;
	this.v = ItemID;
	this.p = PictureID;
