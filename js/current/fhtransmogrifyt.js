...
function DC(v) {
	...
	getObj('Buttons').innerHTML = Adr('window.top.loadwindow2(\'im4.asp?Test=' + Infos[v].v + '&Bonus=0&Material=\',300,300,\'iwindow\',\'' + Infos[v].t + '\');','Info','Info') + (Infos[v].t.indexOf('(E)') == -1 ? '<' + strClicky + ' type=button onclick="if (Processing == 0) {Processing = 1; window.location.replace(\'fhtransmogrifyt2.asp?ItemID=' + Infos[v].v + '&ItemTypeID=' + ItemTypeID + '&P2=' + PageNo + '\');}" style=\'width: 65\'>Select</button><' + strClicky + ' type=button onclick="if (Processing == 0) {Processing = 1; window.location.replace(\'fhtransmogrifyt.asp?InventoryItemID=' + Infos[v].v + '&ItemTypeID=' + ItemTypeID + '&Level=1&P=' + PageNo + '\');}" style=\'width: 65\'>Level 1</button>' : '');
}
