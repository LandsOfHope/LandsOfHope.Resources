...
function AM(Shop2, RDate, Mess, FID, Color, Recipient, PictureID, MDate, Message, MID, xpd, giftid, giftname, giftpicture) {
	...
	if (giftid > 0) {
		document.write('<tr><td><table cellpadding=0 cellspacing=0 class="weakcell"><tr><td valign=top>The following gift is attached:</td><td><table cellpadding=0 cellspacing=0 class="weakcell" width="300" style="border: 1px dotted gold; color: gold; background-color: ' + BGCOLOR_S + '"><tr><td><img src="https://lohcdn.com/game/i/' + giftpicture + '"></td><td><b>' + giftname + '</b><br>Click the button to the right</td><td><' + strClicky2 + ' onclick=\'window.location.replace("?Retrieve=' + MID + '")\'>Retrieve Item</button></td></tr></table></td></tr></table></td></tr>');
	}
}
