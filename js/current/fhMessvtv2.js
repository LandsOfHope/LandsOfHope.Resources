...
if (giftid != 0) {
	document.write('<tr><td><table cellpadding=0 cellspacing=0 class="weakcell"><tr><td valign=top>The following item is attached:</td><td><table cellpadding=0 cellspacing=0 class="weakcell" width="300" style="border: 1px dotted gold; color: gold; background-color: ' + BGCOLOR_S + '"><tr><td><img src="https://res.landsofhope.com/game/i/' + giftpicture + '"></td><td><b>' + giftname + '</b><br>Cost: ' + window.top.BSGM(cost) + '</td><td>' + (giftid > 0 ? '<' + strClicky2 + ' onclick=\'confirm("Are you sure you wish to spend ' + window.top.BSGM(cost) + ' to retrieve ' + giftname + '?",2);\'>Retrieve Item</button><' + strClicky2 + ' onclick=\'confirm("Are you sure you wish to send this item back (no cost)?",3);\'>Return Item</button>' : '') + '</td></tr></table></td></tr></table></td></tr>');
}
}
