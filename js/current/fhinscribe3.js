...
function AvC(Color, z, u, l, a, i, v, PictureID, Itty, eql) {
	...
	document.write('<tr id="I' + IC + '" onmouseover="PC(' + IC + ')" onmouseout="RC(' + IC + ')" onclick="DC(' + IC + ')"><td><img width=15 height=15 src="' + IPath + (PictureID == '' || PictureID == '0' ? 'na.gif' : PictureID) + '"></td><td width="250" style="color: ' + Color + '' + (Level < InscribeLevel ? '; border: 1px dotted red' : '') + '; padding-left: 5px;">' + Itty + '</td><td width="100">' + (Itty.indexOf('(#)') != -1 ? 'Inscribed' : 'Not Inscribed') + '</td></tr>');
	IC = IC + 1;
}
...
function DC(v) {
	getObj('Stuff2').innerHTML = GetTips(v, 0) + '<br>Equip Level: ' + Infos[v].eql + '<br>Inscription Level: ' + InscribeLevel + (Level < InscribeLevel && Math.abs(Infos[v].eql) < Level ? '<br>Inscribing this item will prevent you from equipping it until you reach the right level.' : (Math.abs(Infos[v].eql) < InscribeLevel ? '<br>Inscribing this item will raise its level to ' + InscribeLevel + '.' : ''));
	getObj('Pic').innerHTML = "<img src='" + IPath + (Infos[v].p == '' ? 'na.gif' : Infos[v].p) + "'>";
	getObj('Buttons').innerHTML = '<input type=hidden name=ItemID id=ItemID value=' + Infos[v].z + '>' + (InventoryItemID > 0 ? Adf2('', 'Install Imprint', 'Inscribe') : '') + (Infos[v].t.indexOf('(#)') != -1 ? Adr('getObj(\'ItemID\').value = -' + Infos[v].z + ';getObj(\'inscribe\').submit()', 'Remove Inscription', 'Remove') : '') + Adr('window.top.loadwindow2(\'im4.asp?Test=' + Infos[v].z + '&Bonus=0&Material=\',300,300,\'iwindow\',\'' + Infos[v].t + '\');', 'Info', 'Info') + (TrapID > 0 ? Adr('window.top.loadwindow2(\'Sz.asp?Test=' + TrapID + '\',300,300,\'iwindow\',\'Inscription Info\');', 'Inscription Info', 'Other') : '');
}
