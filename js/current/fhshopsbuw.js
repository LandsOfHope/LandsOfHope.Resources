...
function DC(v) {
getObj('Stuff2').innerHTML = '<b>' + Infos[v].i + '</b><br>Cost: ' + window.top.PSGM(Infos[v].v) + '<br>' + Adr('if (Processing ==0) {Processing = 1; window.location.replace(\'?CharsAt=' + CharsAt + '&ItemID=' +Infos[v].value + '\');}','Train','Train') + Adr('window.top.loadwindow2(\'skz.asp?test=' + Math.abs(Infos[v].value) + '\',300,300,\'iwindow\',\'' + Infos[v].i + '\');','Info','Info') 
...
function PC(v) {
window.top.InfoTip('', '<font color=' + Infos[v].c + '>' + Infos[v].i + '</font><br>Cost: ' + window.top.PSGM(Infos[v].v));
