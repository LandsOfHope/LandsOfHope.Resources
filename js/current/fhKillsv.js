...
function Tipz(v) {
	return '<b>' + Infos[v].i + '</b><br>Classification: ' + Infos[v].rc + '<br>You have killed: ' + Infos[v].s + '<br>' + (Infos[v].qk > 0 ? 'Quest kills: ' + Infos[v].qk + '<br>' : '') + (Infos[v].s > 0 ? 'Last Kill: ' + getdhm(Infos[v].rlk) + ' ago' : '') + (Math.abs(Infos[v].s) < 5000 ? '<br><br><b>' + Infos[v].i + ' Sinker Title</b><br>' + Infos[v].i + ' kills remaining: ' + (5000 - Math.abs(Infos[v].s)) + '<br>' : '') + '';
}

function DC(v) {
	getObj('Stuff2').innerHTML = Tipz(v);
	getObj('Pic').innerHTML = "<img src='" + IPath + (Infos[v].p == '' ? 'na.gif' : Infos[v].p) + "'>";
	getObj('Buttons').innerHTML = (Math.abs(Infos[v].s) >= 5000 ? Adr('confirm(\'Are you sure you wish to use ' + Infos[v].i + ' Sinker as your title?\',' + v + ');', 'Use Sinker Title', 'Enable Title') : '');
}
...
function PromptReturn(returnVal, pb) {
	if (returnVal != null) {
		if (pb != null) {
			var v = pb;
			window.top.Interface.location.replace('fhkillsv.asp?CharsAt=' + Infos[v].r + '&P=' + PageNo);
		}
	}
}