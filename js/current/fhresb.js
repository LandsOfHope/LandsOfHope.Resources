...
function DC(v) {
	...
	getObj('Buttons').innerHTML = (LevelR <= s && Infos[v].q > 0 ? (Infos[v].c == '#ff6666' ? '<font id=tred>Skill not high enough, ' + (Infos[v].l * 5) + ' ' + s2 + ' needed.</font>' : Adr('if (Processing == 0) {Processing = 1; window.location.replace(\'fhresb.asp?CharsAt=' + CharsAt+ '&PetID=' + PetID + '&Material=&Spam=' +Timer + '&A=2&P=1&L=' + (Math.round(Infos[v].l) == 0 ? '1' : Infos[v].l) + '\');}',strword,strword) + Adr('if (Processing == 0) {Processing = 1; window.location.replace(\'fhresb.asp?CharsAt=' + CharsAt+ '&PetID=' + PetID + '&Material=&Spam=' +Timer + '&A=3&P=1&L=' + (Math.round(Infos[v].l) == 0 ? '1' : Infos[v].l) + '\');}','All','All') + '<br>Using the All button will get you the resources with less clicking but you will only gain 1skill up chance for the whole pile and each item will take <b>20 seconds</b>. If you do it individually you will get 1skill up chance for each click of the ' + strword + ' button') : '<font id=tred>Skill not high enough, ' + (Infos[v].l * 5) + ' ' + s2 + ' needed.</font>');
