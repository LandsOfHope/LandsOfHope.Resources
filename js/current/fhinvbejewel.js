...
function DC(v) {
	getObj('Stuff2').innerHTML = '<b>' + Infos[v].i + '</b><br>Bejeweled: ' + (Math.abs(Infos[v].e) != 0 ? 'Yes' : 'No') + (Effect != '' ? '<br>Effect:' + Effect : '');
	getObj('Pic').innerHTML = "<img src='" + IPath + (Infos[v].p == '' ? 'na.gif' : Infos[v].p) + "'>";
	getObj('Buttons').innerHTML = Adr('window.top.loadwindow2(\'im4.asp?Test=' + Infos[v].v + '&Bonus=0&Material=\',300,300,\'iwindow\',\'' + Infos[v].i + '\');','Info','Info') + (Processing == 0 ?  (Math.abs(Infos[v].e) != 0 ? Adr('if (Processing == 0) {Processing = 1;window.location.replace(\'fhinvbejewel.asp?CharsAt=-' + Infos[v].v + '&ItemID=' + ItemID + '&P=' + PageNo + '\');}', 'Uninstall','Uninstall') : '') + (ItemID != 0 ? Adr('if (Processing == 0) {Processing = 1; window.location.replace(\'fhinvbejewel.asp?CharsAt=' + Infos[v].v + '&ItemID=' + ItemID + '&P=' + PageNo + '\')};','Install','Install') : '') : '');
}
