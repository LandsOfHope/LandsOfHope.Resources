...
function GoP(PageNo) {
window.location.replace('?ID=' + ID + '&CharsAt=' + CharsAt + '&P=' + PageNo + '');
}

function DC(v) {
getObj('Stuff2').innerHTML = '<b>' + Infos[v].n + '</b><br>Quantity: ' + Infos[v].q;
getObj('Pic').innerHTML = "<img src='" + IPath + (Infos[v].p == '' ? 'na.gif' : Infos[v].p) + "'>";
getObj('Buttons').innerHTML = Adr('window.parent.loadwindow2(\'im4.asp?Test=' + Infos[v].v + '&Bonus=0&Material=\',300,300,\'iwindow\',\'' + Infos[v].n + '\');', 'Info','Info') + Adr('if (Processing == 0) {Processing = 1; window.location.replace(\'?P=' + PageNo + '&ID=' + ID + '&CharsAt=' + CharsAt + '&Type=' + Infos[v].v + '\');}', 'Give', 'Give');
}
