...
function xCStatus(g, cr, v, tc, tf, tg, tt, ti) {
	...
	if (tt == -1) {
		...
		strout = '<tr><td style="color: white">Status:</td><td colspan=2>' + ap1t + '</td></tr>';
	} else {
		var ap1 = window.parent.GetPerc(tt, tg)
		var ap1t = (tc == 1 ? '<b>COMPLETED</b>' : window.top.PercentBoxR(ap1,'white','' + tg + ' / ' + tt + ''))

		strout = '<tr><td style="color: white">Status:</td><td>' + ap1t + '</td><td>' + ap1 + '%</td></tr>';
	}
	strout = strout + '<tr><td colspan=3>' + Adr('window.parent.loadwindow2(\'imi.asp?Test=' + ti + '&Bonus=0&Material=\',300,300,\'iwindow\',\'Event Requirement\');','Info','Info') + (tf == 0 ? Adr('if (Processing == 0) {Processing = 1; window.location.replace(\'fheventi.asp?ID=' + g + '&CharsAt=' + CharsAt + '\')};','Give Items','Give Items') : '') + '</td></tr>';
