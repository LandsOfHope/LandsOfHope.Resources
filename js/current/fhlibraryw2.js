...
function AvC(Color, lx, isLegend, m, il, mt, PictureID, l, v) {
	if (lx == 'C') { lx = 'Common' }
	...
	Infos[IC] = new newInfo(Color, lx, isLegend, m, il, mt, PictureID, l, v);
...
function newInfo(Color, lx, isLegend, m, il, mt, PictureID, l, v) {
	this.c = Color;
	this.il = il;
	this.p = PictureID;
	this.r = lx;
...
function Tipsfor(v) {
	return '<b>' + Infos[v].m + '</b><br>Rarity: ' + Infos[v].r + (Math.abs(Infos[v].id) > 0 ? '<br><b>Legend Item</b>' : '') + '<br>Skill Name: ' + Infos[v].il + '<br>Material Type: ' + Infos[v].mt + '<br>Minimum Level: ' + Infos[v].l;
}
