...
function AC(t, v, Itty, s, e, m, d, dur) {
	if (Dayz[d] == null) {
		Dayz[d] = new Array();
	}
	if (Dayz[d][0] == null) {
	...
	for (var i = weeks; i > 0; i--) {
		...
		for (var j = 7; j > 0; j--) {
			...
			if (jumped < start || inserted > days) {
				...
				document.write("<table width='100%' class='nav4' cellpadding=0 cellspacing=0><tr><td class='weakercell'><b>" + (tod != 0 && tod == inserted ? '<b>Today</b> (' + inserted + ')' : '' + inserted) + "</b></td></tr>" + DayNumber(inserted, j, tod) + "</table>");
				inserted++;
			}
			document.write("</td>");
		}
...
function DayNumber(dd, jj, tod) {
	...
	if (tod >= dd) {
		strReturn = '<tr height="16"><td width="100%" j=' + jj + ' id="D' + dd + '" onclick="DC(' + dd + ')" style="background-color: #FF6666;">&nbsp;</td></tr>';
	} else {
		if (strReturn == '') {
...
function DDC(stuff) {
	window.top.showPopWin("fhtinytomc.asp?y=" + yy + "&m=" + mm + "&d=" + stuff + "&message=Enter your advertising message:&defaultvalue=0&title=Buy Advertising", 300, 220, PromptReturn, null, "Buy Advertising", stuff);
}
...
function PromptReturn(returnVal, pb, returnVal2) {
	if (returnVal != null) {
		if (pb != null) {
			if (pb > 0) {
				window.location.replace('fhtinytomb.asp?y=' + yy + '&m=' + mm + '&d=' + pb + '&r=' + returnVal);
			}
		}
	}
}
