...
function ShowSubMenu(b) {
	...
	rcspopdiv.style.backgroundColor = "RGB(95, 77, 58)";
	var l = window.top.findPos(getObj('rcsbutton' + b))[0];
	var t = window.top.findPos(getObj('rcsbutton' + b))[1];

	rcspopdiv.style.left = l; //getObj('HEADERX').offsetLeft + getObj('rcsbutton' + b).offsetLeft + getObj('rcsbuttonbox').offsetLeft;
	rcspopdiv.style.top = t + 20; // getObj('rcsbutton' + b).offsetTop + getObj('rcsbuttonbox').offsetTop + 112;
...
function RCSSMOut(b, x) {
	...
	if (bb != null) {
		if (rcsbuttons[b] != null) {
			bb.style.border = '1px solid RGB(95, 77, 58)';
			bb.style.backgroundColor = 'RGB(95, 77, 58)';
		}
	}
}
...
function RCSBOver(b) {
	...
	if (bb != null) {
		if (rcsbuttons[b] != null) {
			bb.style.backgroundImage = `URL("${CDN_RESOURCES_URL}/menubuilder/images/${rcsbuttons[b].picture2}")`;
...
function RCSBOut(b) {
	...
	if (bb != null) {
		if (rcsbuttons[b] != null) {
			bb.style.backgroundImage = `URL("${CDN_RESOURCES_URL}/menubuilder/images/${rcsbuttons[b].picture1}")`;
			clearTimeout(rcsbuttons[b].clicktimer);
		}
	}
}
...
function RCSBClick(b) {
	...
	if (bb != null) {
		if (rcsbuttons[b] != null) {
			bb.style.backgroundImage = `URL("${CDN_RESOURCES_URL}/menubuilder/images/${rcsbuttons[b].picture3}")`;
