...
document.write('<tr width="100%"><td onmouseover="PC(' + FightCounter + ')" onmouseout="RC(this)" width="80px"><img src="' + BigRPath + (Picture == '' ? 'na.gif' : Picture) + '" onerror="onImgErrorSmall(this,\'' + (Picture == '' ? 'na.gif' : Picture) + '\')" width=80 height=80></td><td width="100%" style="color: ' + Color + '" id="Fighter' + FightCounter + '" onclick="DC(' + FightCounter + ')"><table class="weakercell" style="padding-left: 3px" cellpadding=0 cellspacing=0 width="100%"><tr><td colspan=2><b>' + CharName + '</b></td></tr><td>HP:</td><td>' + window.top.PercentBoxSmall(120, hp1p,'red','' + hp2 + '/' + hp1) + '</td></tr><tr><td>EP:</td><td>' + GetPower(cf, pp) + '</td></tr><tr><td colspan=2 id=FightButtons' + FightCounter + '></td></tr></table></td></tr>');
...
w = 10;
for (x = 0; x < p; x++) {
	ret = ret + '<div style=\'float: left; width:' + w + 'px; height: ' + w + 'px;\'><img src=\'https://res.landsofhope.com/game/h/' + f + '.gif\' width=' + w + ' height=' + w + '></div>';
	if (x > 40) {
		ret = ret + '<div style=\'float: left; width:60px; height: ' + w + 'px;\'><img src=\'https://res.landsofhope.com/game/h/' + f + '.gif\' width=' + w + ' height=' + w + '> * ' + (p - 40) + '</div>';
		break;
	}
}
return ret;
...
var ret = '';
var w = 10;
ret = ret + '<div class=\'weakercell\'><center><b>' + p + '</b><img src=\'https://res.landsofhope.com/game/h/' + f + '.gif\' width=' + w + ' height=' + w + '></center></div>';
return ret;
}
...
function GetAbilities(f) {
	...
	if (Fighters[f].s == Side && f == 0) {
		retVal = retVal + '<div class="btn" onclick="FAC(' + f + ', -10)" style="float: left; width: 34px; height: 40px; background-image: URL(https://res.landsofhope.com/game/icons/lightning.png); background-repeat: no-repeat; background-position: center center;" title="Ask for your Gods Favor" onmouseover="PC3(this)" onmouseout="RC2(this)" align=bottom>&nbsp;</div>'
		retVal = retVal + '<div class="btn" onclick="FAC(' + f + ', -20)" style="float: left; width: 34px; height: 40px; background-image: URL(https://res.landsofhope.com/game/icons/star.png); background-repeat: no-repeat; background-position: center center;" title="Entertain" onmouseover="PC3(this)" onmouseout="RC2(this)" align=bottom>&nbsp;</div>'

		for (x = 0; x < Fighters[f].abilities.length; x++) {
			retVal = retVal + '<div class="btn" onclick="FAC(' + f + ', ' + Fighters[f].abilities[x].id + ')" style="float: left; width: 34px; height: 40px; color: white;" title="' + Fighters[f].abilities[x].name + '" onmouseover="PC2(this,' + f + ',' + x + ')" onmouseout="RC2(this)" align=center valign=bottom><table cellpadding=0 cellspacing=0 width="32"><tr><td><center><img src="https://res.landsofhope.com/game/dp/' + Fighters[f].abilities[x].pp + '" width=20 height=20></center></td></tr><tr><td>' + GetPower2(Fighters[f].cf, Fighters[f].abilities[x].cost) + '</td></tr></table></div>'
		}
	}
	return retVal;
}
...
window.top.InfoTip('https://res.landsofhope.com/game/dp/' + Fighters[f].abilities[x].pp + '', '<table class="weakercell" style="padding-left: 3px" cellpadding=0 cellspacing=0><tr><td><b>' + Fighters[f].abilities[x].name + ' (' + Fighters[f].abilities[x].sub + ')</b></td></tr><tr><td>Cost: ' + Fighters[f].abilities[x].cost + ' ' + Fighters[f].abilities[x].powertype + '</td></tr></table>');
