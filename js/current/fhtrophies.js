...
function AvC(tid, tin, tip, ot, ts) {
	...
	document.write(`<tr id="T${TC}" style="color:${Color}" onclick="DC(${TC})" onmouseover="PC(${TC});" onmouseout="RC(this)"><td width="40"><img src="${CDN_RESOURCES_URL}/game/i/${tip}"></td><td width="260" valign=top><b>${tin}</b><br><font id=tmagenta>${ot}</font></td><td>Rank ${ts}</td></tr>`);
	TC = TC + 1;
