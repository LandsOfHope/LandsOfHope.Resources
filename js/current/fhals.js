...
function CS(how, stuff, mc) {
	if (Processing == 0) {
		Processing = 1;
		window.frames['ResultsOfit'].location.replace('FHSpells6.asp?Type=' + how + '&CharsAt=' + stuff + '');
	}
}
...
function DC(v) {
	...
	getObj('Buttons').innerHTML = Adr('CS(0, ' + Infos[v].v + ', ' + Infos[v].mc + ');', 'Cast', 'Cast') + Adr('window.parent.loadwindow2(\'Mz.asp?test=' + Infos[v].v + '\',300, 250, \'iwindow\',\'' + Infos[v].s + '\');', 'Info', 'Info');
}
