...
function DC(v) {
	if (Processing == 0) {
		getObj('Stuff2').innerHTML = '<font class="weakcell" style="color:' + Infos[v].c + '"><b>' + (Infos[v].t.length > 28 ? Infos[v].t.substr(0, 28) + '..' : Infos[v].t) + '</b></font><br>Rarity: ' + Infos[v].l + '<br>Level: ' + Infos[v].v;
		getObj('Pic').innerHTML = "<img src='" + IPath + (Infos[v].p == '' ? 'na.gif' : Infos[v].p) + "'>";
		getObj('Buttons').innerHTML = '' + (Processing == 0 ? '<' + strClicky3 + ' onclick="if (Processing == 0) {Processing = 1; window.location.replace(\'?P=' + PageNo + '&ItemTypeID=' + ItemTypeID + '&ItemID=1&Type=' + Type2 + '&InventoryItemID=' + Infos[v].z + '\');}" style=\'width: 85\'>Disguise</button>' + Adr('window.top.loadwindow2(\'im4.asp?Test=' + Infos[v].z + '&Bonus=0&Material=\',300,300,\'iwindow\',\'' + Infos[v].t + '\');', 'Info', 'Info') : '');
	}
}
