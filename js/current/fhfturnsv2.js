...
function PC(v) {
window.top.InfoTip('', '<b>' + Choices[v].i + '</b><br>' + (Choices[v].q == -1 || Choices[v].q == -2 ? 'Cures: ' + Math.abs(Choices[v].q) + (Choices[v].q == -1 ? ' disease' : ' poison') : 'Heals: ' + Math.abs(Choices[v].q)) + '<br>Mana Cost: ' + Choices[v].m + (Choices[v].dot != 0 ? '<br><i>Effect Over Time</i>' : ''));
