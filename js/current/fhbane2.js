...
function AvC(Color, z, u, l, a, i, v, PictureID, Itty, eql, r) {
if (l == 'C') {l = 'Common'}
...
Infos[IC] = new newInfo(Color, z, u, l, a, i, v, PictureID, Itty, eql, r);
document.write('<tr id="I' + IC + '" onmouseover="PC(' + IC + ')" onmouseout="RC(' + IC + ')" onclick="DC(' + IC + ')"><td><img width=15 height=15 src="' + IPath + (PictureID == '' || PictureID == '0' ? 'na.gif' : PictureID) + '"></td><td width="250" style="color: ' + Color + '; padding-left: 5px;">' + Itty + '</td><td width="100">' + (r != 0 ? 'Baned' : '') + '</td></tr>');
IC = IC + 1;
}

function newInfo(Color, z, u, l, a, i, v, PictureID, Itty, eql, r) {
this.c = Color;
this.eql = eql;
this.p = PictureID;
this.t = Itty;
...
function DC(v) {
getObj('Stuff2').innerHTML = GetTips(v, 0) + '<br>Equip Level: ' + Infos[v].eql + '';
getObj('Pic').innerHTML = "<img src='" + IPath + (Infos[v].p == '' ? 'na.gif' : Infos[v].p) + "'>";
getObj('Buttons').innerHTML = '<input type=hidden name=ItemID id=ItemID value=' + Infos[v].z + '>' + (InventoryItemID > 0 ? Adf2('', 'Install bane', 'Install') : '') + (Infos[v].r != 0 ? Adr('getObj(\'ItemID\').value = -' + Infos[v].z + ';getObj(\'inscribe\').submit()', 'Remove Bane', 'Remove') : '') + Adr('window.top.loadwindow2(\'im4.asp?Test=' + Infos[v].z + '&Bonus=0&Material=\',300,300,\'iwindow\',\'' + Infos[v].t + '\');','Info','Info');
}
