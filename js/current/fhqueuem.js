...
function DC(v) {
getObj('Stuff2').innerHTML = '<b>' + Infos[v].i + '</b><br>Level: ' + Infos[v].v + '<br>Items on Queue: '  + Infos[v].cc +'<br>' + Adr('if (Processing ==0) {Processing = 1; window.location.replace(\'fhqueue.asp?CharsAt=' + Infos[v].value + '\');}','Show ' + Infos[v].i + '`s queue','Show');
...
function AM(Named, ShopID, value, Picture, cc) {
var Color = LITE;
if (Infos[IC] == null) {
	Infos[IC] = new Array();
}
Infos[IC] = new newInfo(Color, Named, ShopID, value, Picture, cc);
IC = IC + 1;
}

function newInfo(Color, Named, ShopID, value, Picture, cc) {
this.c = Color;
this.v = value;
this.p = Picture;
this.i = Named;
