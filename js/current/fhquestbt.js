...
function DC(v) {
getObj('Stuff2').innerHTML = '<b>' + Infos[v].qn + '</b><img src="' + IPath + (Infos[v].p == '' ? 'na.gif' : Infos[v].p) + '" style="float: left;padding: 2px; margin: 2px;"><br>Quest Level: ' + Infos[v].ql + '<br>If you wish to take this trophy quest go to <b>' + Infos[v].cn + '</b> at <b>' + Infos[v].x + ', ' + Infos[v].y + '</b>' + (Math.abs(Infos[v].b) != 0 ? ' and look inside the buildings, you may need to explore a building if you can not find the NPC immediately' : '') + '.';
}
...
function AM(ql, v, x,y, Named, Picture, CN, b) {
var Color = LITE;
var bgx = ''
...
Infos[IC] = new newInfo(Color, ql, v, x,y, Named, Picture, CN, b, bgx);
IC = IC + 1;
}

function newInfo(Color, ql, v, x,y, Named, Picture, CN, b, bgx) {
this.c = Color;
this.b = b;
this.p = Picture;
this.bg = bgx;
