...
function GoP(PageNo) {
	window.location.replace('fhwantv.asp?CharsAt=' + CharsAt + '&P=' + PageNo + '');
}
...
function PromptReturn(returnVal, pb) {
	...
	if (returnVal != null && pb != null && Processing == 0 && LastV >= 0) {
		if (pb == 1) {
			Processing = 1;
			window.top.Interface.location.replace('fhwantv.asp?CharsAt=' + CharsAt + '&P=' + PageNo + '&Times=1&ItemID=' + Infos[v].v + '&cat=' + Cat);
		} else {
			if (returnVal >= 1) {
				var cq = Math.floor(Infos[v].q / qq);
				if (cq > returnVal) {
				...
				Processing = 1;
				window.top.Interface.location.replace('fhwantv.asp?CharsAt=' + CharsAt + '&P=' + PageNo + '&Times=' + returnVal + '&ItemID=' + Infos[v].v + '&cat=' + Cat);
			}
		}
	}
}
