...
function PromptReturn(returnVal, pb) {
	if (returnVal != null) {
		if (pb != null) {
			window.top.Interface.Interface.location.replace('fhguem2.asp?P=' + PageNo + '&Mask=' + Mask + '&CharsAt=' + Infos[LastV].v + '&Type=' + pb + '');
		}
	}
}
...
function DC(v) {
	...
	getObj('Stuff2').innerHTML = '<b>' + Infos[v].t + '</b><br>New Member - Promote to Guild Level or Kick';
	getObj('Buttons').innerHTML = Adr('window.parent.focus();window.parent.location.replace(\'fhMess.asp?CharsAt=-' + Infos[v].v + '\');', 'Private message ' + Infos[v].t, 'PM') + Adr('window.location.replace(\'?P=' + PageNo + '&Mask=' + Mask + '&CharsAt=' + Infos[v].v + '&Type=-2\');', 'Kick ' + Infos[v].t + ' from guild.', 'Kick');
}
