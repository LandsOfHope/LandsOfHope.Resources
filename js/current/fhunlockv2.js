...
function DC(v) {
	getObj('Stuff2').innerHTML = '<b>' + Infos[v].i + '</b><br>Lockpicking Required: ' + ((Infos[v].s * 5) - 5) + '<br>Material Level: ' + Infos[v].l + '';
	getObj('Pic').innerHTML = "<img src='" + IPath + (Infos[v].p == '' ? 'na.gif' : Infos[v].p) + "'>";
	getObj('Buttons').innerHTML = Adr('window.top.loadwindow2(\'im4.asp?Test=' + Infos[v].v + '&Bonus=0&Material=\',300,300,\'iwindow\',\'' + Infos[v].i + '\');', 'Info', 'Info') + (Processing == 0 ? (Skill >= (Math.abs(Infos[v].s * 5) - 5) ? '<' + strClicky + ' onclick="if (Processing == 0) {Processing = 1; window.location.replace(\'fhunlock.asp?Type=1&L=' + Infos[v].s + '&ItemID=' + Infos[v].v + '&P=' + PageNo + '\')};" style=\'width: 65\'>Unlock</button>' : '') : '');
}
