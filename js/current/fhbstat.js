...
function DC(v) {
	...
	if (Processing == 0) {
		getObj('Buttons').innerHTML = Adr('if (Processing == 0) {confirm(\'Erect a statue of ' + Infos[v].t + ' in this room. Continue ?\',' + v + ');}', 'Erect a statue of ' + Infos[v].t, 'Erect');
	} else {
...
function PromptReturn(returnVal, pb) {
	if (returnVal != null) {
		if (pb != null && Processing == 0) {
			Processing = 1;
			window.top.Interface.location.replace('fhbstat.asp?Action=1&Mask=' + Mask + '&CharsAt=' + CharsAt + '&Type=' + Infos[pb].v);
		}
	}
}
