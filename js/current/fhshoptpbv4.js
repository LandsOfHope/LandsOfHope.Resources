...
function DC(v) {
	getObj('Stuff2').innerHTML = '<b>' + Infos[v].i + '</b>' + (Infos[v].c == 'gold' ? '<br><b style=\'color: gold;\'>Vessel Item</b>' : '') + '<br>Rarity: ' + Infos[v].q + '<br>Skill: ' + Infos[v].s + '<br>Cost: ' + window.top.BSGM(Infos[v].v) + '<br>' + Adr('if (Processing ==0) {Processing = 1; window.location.replace(\'?CharsAt=' + CharsAt + '&P=' + PageNo + '&ItemID=' + Infos[v].value + '\');}', 'Buy', 'Buy') + Adr('window.top.loadwindow2(\'mr.asp?ItemID=' + Math.abs(Infos[v].value) + '\',300,300,\'iwindow\',\'' + Infos[v].i + '\');', 'Craft Info', 'Craft Info') + (Infos[v].b > 0 ? Adr('window.top.loadwindow2(\'imi' + (Infos[v].c == 'gold' ? 'v' : '') + '.asp?Test=' + Infos[v].b + '\',300,300,\'iwindow\',\'' + Infos[v].i + '\');', 'Item Info', 'Info') : '')
