...
function DC(v) {
	...
	getObj('Buttons').innerHTML = (Infos[v].iid != 0 ? Adr('window.parent.loadwindow2(\'' + (Infos[v].iid < 0 ? 'mr.asp?ItemID=' + Math.abs(Infos[v].iid) : 'im' + (Infos[v].tt == 10 ? 'iv' : '3') + '.asp?Test=' + Infos[v].iid + '&Bonus=0&Material=') + '\',300,300,\'iwindow\',\'Trophy Reward\');', 'Reward Info', 'Reward Info') : '') + (Math.abs(Infos[v].u) != 0 ? '' : Adr('if (Processing == 0) {Processing = 1; window.location.replace(\'fhtrophyi.asp?ID=' + (Infos[v].gtid == 0 ? '-' + Infos[v].tid : '' + Infos[v].gtid) + '&CharsAt=' + CharsAt + '\');}', 'Give Items', 'Give Items'));
}
