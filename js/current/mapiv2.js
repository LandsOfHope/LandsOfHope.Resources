...
function BuildSearch() {
	...
	if (GameID == 20) {
		str += '' + BBBS(-373, '6684.gif', 'Godly Things', 1)
	}
	str += '' + BBBS(-104, 'ba.gif', 'Bags and Boxes', 1)
	str += '' + BBBS(-311, 'soil.gif', 'Soil and Seeds', 1)
	str += '' + BBBS(-50, 'me1.gif', 'Treasure Maps', 100)
	str += '' + BBBS(-250, 'ycc.gif', 'Crystals and Herbs', 500)
	str += '' + BBBS(-256, 'sc1.gif', 'Battered Chest', 1000)
	if (GameID == 13) {
		str += '' + BBBS(-114, 'musr14.gif', 'Mushrooms (Juratania)', 3000)
	}
	if (GameID == 14) {
		str += '' + BBBS(-263, 'crack5.gif', 'Unstable Areas', 250)
		str += '' + BBBS(-213, 'soil3.gif', 'Rare Resources', 50)
		str += '' + BBBS(-117, 'wreck.gif', 'Wreckage', 1)
	}
	//alert(str);
	var v = 0;
	var i = 0;
...
function sv2(v) {
	window.top.InfoTip('' + IPath + Groups[v].p, '<b>' + Groups[v].t + '</b><br>You need ' + Groups[v].sv + ' track items skill to use this ability.')
