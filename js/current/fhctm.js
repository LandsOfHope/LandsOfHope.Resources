...
function DC(v) {
	...
	getObj('Buttons').innerHTML = '' + (q3 <= 1000 ? Adr('if (Processing == 0) {Processing = 1; window.location.replace(\'fhctm.asp?CharsAt=' + CharsAt + '&ItemID=' + Infos[v].z + '\');}', 'Sell', 'Sell') : Adr('if (Processing == 0) {Processing = 1; window.location.replace(\'fhctm.asp?CharsAt=' + CharsAt + '&aok=1000&ItemID=' + Infos[v].z + '\');', 'Sell', 'Sell')) + (Infos[v].i.indexOf('(?)') == -1 ? Adr('window.top.loadwindow2(\'im4.asp?Test=' + Infos[v].z + '&Bonus=0&Material=\',300,300,\'iwindow\',\'' + Infos[v].i + '\');', 'Info', 'Info') : '');
}
