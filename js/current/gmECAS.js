...
function DC(v) {
	LastV = v;
	getObj('Stuff2').innerHTML = '<b>' + Infos[v].t + '</b><br>Player can Cancel: ' + (Infos[v].b == 0 ? 'No' : 'Yes') + '<br>Casted: ' + Infos[v].d + '<br>Duration: ' + Infos[v].i + '';
	getObj('Pic').innerHTML = "<img src='" + Infos[v].p + "'>"
	if (Processing == 0) {
		getObj('Buttons').innerHTML = Adr('window.top.loadwindow2(\'Mz.asp?Test=' + Infos[v].m + '&Bonus=0&Material=\',300,300,\'iwindow\',\'' + Infos[v].t + '\');', 'Info', 'Info') + (Infos[v].t.indexOf('*Broken Spell*') == -1 ? Adr('Processing=1;window.location.replace(\'?CharsAt=' + CharID + '&P=' + PageNo + '&ItemID=1&InventoryItemID=' + Infos[v].m + '\');', 'Cancel', 'Cancel') + Adr('conf(\'Please enter the new Duration for this spell (minutes)\', \'' + Infos[v].i + '\', \'/gm/gmecas.asp?CharsAt=' + CharID + '&P=' + PageNo + '&InventoryItemID=' + Infos[v].v + '&ItemID=-\');', 'Duration', 'Duration') : Adr('Processing=1;window.location.replace(\'?CharsAt=' + CharID + '&P=' + PageNo + '&ItemID=2&InventoryItemID=' + Infos[v].m + '\');', 'Remove', 'Remove'));
	} else {
