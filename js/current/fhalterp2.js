...
function DC(v) {
getObj('Stuff2').innerHTML = '<b>' + Infos[v].i + '</b><br>Level: ' + Infos[v].v + '<br>Race: ' + Infos[v].r + '<br>' + Adr('if (Processing ==0) {Processing = 1; window.location.replace(\'fhalterp3.asp?CharsAt=' + Infos[v].value + '\');}','Select ' + Infos[v].i ,'Select');
...
function PC(v) {
window.top.InfoTip('' + IPath + Infos[v].p, ' <font color=' + Infos[v].c + '>' + Infos[v].i + '</font><br>Level: ' + Infos[v].v + '<br>Race: ' + Infos[v].r);
...
function AM(Named, ShopID, value, Picture, r) {
var Color = LITE;
if (Infos[IC] == null) {
	Infos[IC] = new Array();
}
Infos[IC] = new newInfo(Color, Named, ShopID, value, Picture, r);
IC = IC + 1;
}

function newInfo(Color, Named, ShopID, value, Picture, r) {
this.c = Color;
this.v = value;
this.p = Picture;
this.i = Named;
