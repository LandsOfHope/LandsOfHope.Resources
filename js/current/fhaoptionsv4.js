var JW = JW;
const CDN_RESOURCES_URL_fhaoptionsv4 = new URL(document.currentScript.src).origin;
...
function Themes2(tx) {
	...
	for (x = 0; x < dimarr.length; x++) {
		var xw1 = dimarr[x];
		strout += `<td ${strSSClick} ${tx == xw1 ? 'style=\'border: 1px solid gold\'' : 'style=\'border: 1px solid RGB(115,102,86)\' '} align=center><img title="Theme: ${xw1}" src="${CDN_RESOURCES_URL_fhaoptionsv4}/${xw1}.gif" onclick="SetTheme('${xw1}');"></td>`;
		y = y + 1;
		if (y == 2) {
			strout += '</tr><tr>';
			y = 1;
