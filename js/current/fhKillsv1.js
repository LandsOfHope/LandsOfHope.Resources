...
function DC(v) {
	...
	getObj('Buttons').innerHTML = (Math.abs(Infos[v].s) >= 5000 ? Adr('confirm(\'Are you sure you wish to use ' + Infos[v].i + ' Slayer as your title?\',' + v + ');','Use Slayer Title','Enable Title') + Adr('window.top.Interface.location.replace(\'fhkills.asp?Type=2&CharsAt=' +Infos[v].r + '&P=' + PageNo + '\');','Use Slayer Form','Slayer Form') : '');
}
...
function PromptReturn(returnVal, pb) {
	if (returnVal != null) {
		if (pb != null) {
			...
			window.top.Interface.location.replace('fhkills.asp?CharsAt=' +Infos[v].r + '&P=' + PageNo);
		}
	}
}