...
function AM(PictureID, pp,QuestName, Color, Quest, QuestID, Spe) {
var bgx = '';
...
Infos[IC] = new newInfo(Color, PictureID, pp,QuestName, Quest, QuestID, Spe, bgx);
IC = IC + 1;
}

function newInfo(Color, PictureID, pp,QuestName, Quest, QuestID, Spe, bgx) {
this.c = Color;
this.s = Spe;
this.p = PictureID;
this.pp = pp;
...
function DC(v) {
	window.frames['ResultsOfit'].location.replace('fhquest2.asp?P=' + PageNo + '&CharsAt=' + Infos[v].q);
}

function GetRow(v) {
	if (Infos[v].type == 1) {
		return '<tr id="I' + v + '" onmouseover="PC(' + v + ')" onmouseout="RC(' + v + ')" onclick="DC(' + v + ')" ' + (Infos[v].b != '' ? 'style="background-color: ' + Infos[v].b + '"' : '') + '><td width=15><img src=\'' +  window.top.FHIP + Infos[v].pp + '/' + Infos[v].p + '\' width=15 height=15></td><td width="280" style="color: ' + Infos[v].c + '; padding-left: 5px;">' + (Infos[v].s == 1 ? '<i>' + Infos[v].qn + '</i>' : Infos[v].qn) + '</b></td></tr>';
	} else {
...
function PC(v) {
window.top.InfoTip('' + window.top.FHIP + Infos[v].pp + '/' +Infos[v].p , '' + Infos[v].qn + '');
